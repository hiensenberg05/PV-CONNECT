...
async def process_image_node(state: GraphState) -> GraphState:
    """
    Handles prescription / document uploads using Ollama Vision:
    1) Get image bytes (from media_url or direct upload)
    2) Upload to Cloudinary (optional)
    3) Run Ollama Vision (llama3.2-vision or similar) to extract drug/dosage/frequency/doctor/clinic
    ...
    # Use Ollama Vision for OCR (supports vision models like llama3.2-vision)
    try:
        model = get_model("llama3.2-vision")  # Ollama vision model
...
- Return valid JSON only, no markdown, no code blocks"""
        ...
        # Open image and process with Ollama Vision
        ...
        logger.error(f"Failed to parse OCR JSON: {e}, response: {response.text[:200] if hasattr(response, 'text') else 'N/A'}")
        ocr_data = {}
    except Exception as e:
